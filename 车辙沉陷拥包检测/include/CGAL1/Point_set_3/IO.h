  ...
  CGAL::Point_set_3<Point, Vector>& point_set)  ///< point set
{
  ...
  //这里也非常坑，有可能是true，也有可能是false，  不用深究，  执行里面的内容就对了
//   if (remove_R)
//     {
      ...
      if (foundR && foundG && foundB)
        {
          for (typename Point_set::iterator it = point_set.begin(); it != point_set.end(); ++ it)
            {
              put (R, *it, (unsigned short)(255 * get(charR, *it)));
              put (G, *it, (unsigned short)(255 * get(charG, *it)));
              put (B, *it, (unsigned short)(255 * get(charB, *it)));
            }
        }
//    }
