...
private:
  ...
  mutable std::map<Feature_handle, std::size_t> m_map_features;  // 特征对应特征索引
...
*/
  ...
  */
  ...
  float train (const LabelIndexRange& ground_truth,  // 类型索引 
               unsigned int nb_tests = 300)  // 训练参数
  {
    std::vector<std::vector<std::size_t> > training_sets (m_labels.size()); // 类型索引内嵌套点索引
    std::size_t nb_tot = 0;  // 已经分类的个数
    ...
    for (std::size_t i = 0; i < m_labels.size(); ++ i)
      CGAL::cpp98::random_shuffle (training_sets[i].begin(), training_sets[i].end());  // 打乱点顺序
#endif
    
    CGAL_CLASSIFICATION_CERR << "Training using " << nb_tot << " inliers" << std::endl;
    
    for (std::size_t i = 0; i < m_labels.size(); ++ i)  // 如果训练集小于标签个数，或者此训练集内容为空，提醒训练集缺少
      if (training_sets.size() <= i || training_sets[i].empty())
        std::cerr << "WARNING: \"" << m_labels[i]->name() << "\" doesn't have a training set." << std::endl;

    std::vector<float> best_weights (m_features.size(), 1.);  // 每个特征的最佳权重

    std::vector<Feature_training> feature_train;  // 特征训练
    std::size_t nb_trials = 100;
    float wmin = 1e-5f, wmax = 1e5f;  // -100000  100000
    float factor = std::pow (wmax/wmin, 1.f / float(nb_trials));  //-1,  1    == -1
    
    // 遍历每个特征
    for (std::size_t j = 0; j < m_features.size(); ++ j)
    {
      Feature_handle feature = m_features[j];
      best_weights[j] = weight(j);  // 最佳权重给个默认值

      std::size_t nb_useful = 0;
      float min = (std::numeric_limits<float>::max)();
      float max = -(std::numeric_limits<float>::max)();

      set_weight(j, wmin);  // 给当前特征设置一个最小的权重
      for (std::size_t i = 0; i < 100; ++ i)  // 计算一百次
      {
        estimate_feature_effect(j, training_sets);  // 计算特征对标签的影响
        if (feature_useful(j))
        {
    ...
    for (std::size_t i = 0; i < feature_train.size(); ++ i)
    {
      ...
      for (std::size_t j = 0; j < m_features.size(); ++ j)
      {
        ...
        set_weight(j, best_weights[j]);
        estimate_feature_effect(j, training_sets);  // 估计特征对标签的影响
        if (feature_useful(j))
          nb_used ++;
        else
          set_weight(j, 0.);
