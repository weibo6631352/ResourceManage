...
#include <CoorConv.hpp>
 ...
 void pct::io::Load_las_off(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud, std::string path)
 {
	...
	bool first = true;
 	...
 	while (lasreader->read_point())
 	{
		...
		if (true == first)
		{
			std::cout << "first point:" << lasreader->point.get_x() << " " << lasreader->point.get_y() << " " << lasreader->point.get_z() << std::endl;
			first = false;
		}
 ...
 void pct::io::save_las_off(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud, std::string path)
 {
	Setting &setting = Setting::ins();
 	...
 	{
 		laspoint.set_x(it->x+setting.global_offset_[0]);
 		laspoint.set_y(it->y+setting.global_offset_[1]);
 		laspoint.set_z(it->z+setting.global_offset_[2]);
