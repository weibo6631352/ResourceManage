...
//   Scene_points_with_normal_item(const Polyhedron& input_mesh);
  ...
  virtual bool supportsRenderingMode(RenderingMode m) const Q_DECL_OVERRIDE{ return false; };
...
//   int getPointSliderValue();
  void selectPoints(std::vector<Point_set::Index>& selected, std::vector<Point_set::Index>& unselected);
public Q_SLOTS:
  // Delete selection
