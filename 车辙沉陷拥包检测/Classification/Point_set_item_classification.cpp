...
Point_set_item_classification::Point_set_item_classification(Scene_points_with_normal_item* points)
  : m_points (points),
    m_generator (NULL)
{
    std::cerr << "Point_set_item_classification!" << std::endl;
  m_index_color = 1;
...
void Point_set_item_classification::change_points_real_color()
{
    Point_set::Property_map<unsigned char> redc, greenc, bluec;
    auto pointset = m_points->point_set();

    for (Point_set::const_iterator it = pointset->begin();
        it != pointset->first_selected(); ++it)
    {
        QColor color(0, 0, 0);
        std::size_t c = m_classif[*it];  // 类型c

        if (c != std::size_t(-1))  // c不是未分类
        {
            color = m_label_colors[c];  // 找出c分类对应的color
        }
        
        pointset->set_color(*it, color.red(), color.green(), color.blue() );
    }
}
...
void Point_set_item_classification::change_color (int index)
{
  ...
  ramp.build_rainbow();
  reset_indices();  // 重置点云索引
  if (index_color == -1) // item color
    {
        // point_set应该是按未选择、已选则的顺序排序
        // 从第一个点到第一个选择的点
      for (Point_set::const_iterator it = m_points->point_set()->begin();
           it != m_points->point_set()->first_selected(); ++ it)
        {
  ...
  else if (index_color == 2) // training
    {
      for (Point_set::const_iterator it = m_points->point_set()->begin();
        ...
        {
          QColor color (0, 0, 0);
          int c = m_training[*it];  // 训练的类型
          int c2 = m_classif[*it];  // 分类的类型
          
          if (c != -1)  // 是否在训练集中
            color = m_label_colors[std::size_t(c)];  // 通过训练集得到对应颜色
          
          float div = 1;
          if (c != c2)  // 如果结果分类与训练分类不一致
            div = 2;  // 颜色因子=2，  其实是加重了颜色
    ...
    {
      // 特征显示视图
      Feature_handle feature = m_features[index_color - 3];  // 根据特征的索引得到特征
...
void Point_set_item_classification::reset_indices ()
{
  ...
  Point_set::Index idx;  // Index初始是-1
  ++ idx;
  for (std::size_t i = 0; i < m_points->point_set()->size(); ++ i)
    *(indices.begin() + i) = idx ++;  // 点云索引从0开始建立
}
...
void Point_set_item_classification::compute_features (std::size_t nb_scales)
{
  ...
  std::cerr << "compute_features" <<
      "hasnormals" << normals << 
      "hascolors" << colors <<
      "hasecho" << echo << std::endl;

  if (!normals && !colors && !echo)
    m_generator = new Generator (m_features, *(m_points->point_set()), m_points->point_set()->point_map(), nb_scales);
  else if (!normals && !colors && echo)  // 没颜色的走这个分支
    m_generator = new Generator (/*特征*/m_features, 
    /*点集*/*(m_points->point_set()),
    /*属性集*/m_points->point_set()->point_map(), 
    /* */nb_scales,
    /*法线*/CGAL::Default(), 
    /*颜色*/CGAL::Default(), 
    /* */echo_map);
  ...
  else if (!normals && colors && echo)  // 有颜色的走这个分支
    m_generator = new Generator (m_features, *(m_points->point_set()), m_points->point_set()->point_map(), nb_scales,
  ...
  // 加载xml训练文件后会读取权重和特征
  // 用计算出来的特征和label初始化分类器
  delete m_sowf;
  m_sowf = new Sum_of_weighted_features (m_labels, m_features);  // 初始加权特征分类器
  delete m_ethz;
  m_ethz = new ETHZ_random_forest (m_labels, m_features);  // 随机森林分类器
#ifdef CGAL_LINKED_WITH_OPENCV
  delete m_random_forest;
  m_random_forest = new Random_forest (m_labels, m_features);  // opencv随机森林分类器
#endif
                                          ...
                                          std::size_t num_trees, std::size_t max_depth)
{
  ...
  std::vector<int> training (m_points->point_set()->size(), -1); //训练样本，对应每一个点
  std::vector<int> indices (m_points->point_set()->size(), -1); //每一个点的索引

  std::vector<std::size_t> nb_label (m_labels.size(), 0);
  std::size_t nb_total = 0;
  
  // point_set应该是按没选择，以选择排序的
  // 处理从第一个没选择的点，到第一个选择的点
  // *it内容是索引
  for (Point_set::const_iterator it = m_points->point_set()->begin();
       it != m_points->point_set()->first_selected(); ++ it)
  {
    training[*it] = m_training[*it]; //训练样本的类型值
    if (training[*it] != -1) //如果已分类
    {
      nb_label[std::size_t(training[*it])] ++; //此类样本个数++ 
      ++ nb_total; //总训练样本的个数
    }
  }
  ...
  if (classifier == 0)
    {
      m_sowf->train<Concurrency_tag>(training, nb_trials); //训练样本，最大训练深度
