...
#include <CGAL1/Point_set_3/IO.h>
...
void Scene_points_with_normal_item::selectPoints(std::vector<Point_set::Index>& selected, std::vector<Point_set::Index>& unselected)
{
    Point_set* points = point_set();
    points->unselect_all();
    ...
    points->set_first_selected(points->begin() + unselected.size());
}
