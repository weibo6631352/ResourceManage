...
public:
  ...
  char cred(const Index& index) const
  {
      return (m_red == Byte_map()) ? m_fred[index]*255 : double(m_red[index]);
  }
  double cgreen(const Index& index) const
  {
      return (m_green == Byte_map()) ? m_fgreen[index] * 255 : double(m_green[index]);
  }
  double cblue(const Index& index) const
  {
      return (m_blue == Byte_map()) ? m_fblue[index] * 255 : double(m_blue[index]);
  }
  ...
  void set_color2(const Index& index, unsigned char r, unsigned char g, unsigned char b)
  {
      m_red[index] = r;
      m_green[index] = g;
      m_blue[index] = b;
      m_fred[index] = r / 255;
      m_fgreen[index] = g / 255;
      m_fblue[index] = b / 255;
  }

    
  // m_indices是根据无选择，选择这么个顺序排的序
