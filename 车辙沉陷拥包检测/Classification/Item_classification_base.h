...
{
public:
  typedef CGAL::Classification::Label_handle   Label_handle; //标签助手
  typedef CGAL::Classification::Feature_handle Feature_handle; //特征助手
  typedef CGAL::Classification::Label_set   Label_set; //标签集合
  typedef CGAL::Classification::Feature_set Feature_set; //特征集合
  typedef CGAL::Classification::Sum_of_weighted_features_classifier Sum_of_weighted_features; //自定义特征分类器
  typedef CGAL::Classification::ETHZ_random_forest_classifier ETHZ_random_forest; //随机森林分类器
...
// #endif
  ...
  virtual QColor add_new_label(const char* name, QColor c)
  {
      m_labels.add(name);
      m_label_colors.push_back(c);
...
protected:
  ...
  Label_set m_labels;  //标签集合
  Feature_set m_features;  // 特征集合
  std::vector<QColor> m_label_colors;  // 标签颜色
  Sum_of_weighted_features* m_sowf;  // 多种混合权重特征
  ETHZ_random_forest* m_ethz;  // 随机森林
#ifdef CGAL_LINKED_WITH_OPENCV
